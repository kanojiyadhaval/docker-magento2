...
copy-in: ## Copy project files in the "app" volume
	docker run --rm \
		...
		busybox sh -c "cp -R /project/* /var/www/html/"
	make permissions
...
permissions: ## Fix permissions in the "php" container
	docker-compose exec php sh -c "chown -R www-data ./"
...
.PHONY: backup build copy-in composer logs logs-full nginx permissions php ps restart restore start stats stop yarn
